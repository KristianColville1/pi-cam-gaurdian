...
export function buildPathsObject(operations) {
    ...
    for (const operation of operations) {
        ...
        const operationObj = {};
        if (docs?.summary)
            operationObj.summary = docs.summary;
        if (docs?.description)
            operationObj.description = docs.description;
        if (docs?.tags)
            operationObj.tags = docs.tags;
        if (docs?.operationId)
            operationObj.operationId = docs.operationId;
...
export function buildResponses(docs) {
    const responses = {};
    const entries = docs?.responses && Object.keys(docs.responses).length > 0
...
function convertSchemaToParameters(schemaLike, location) {
    ...
    const params = [];
    const requiredSet = new Set(Array.isArray(schemaObject.required) ? schemaObject.required : []);
